...
export function MagicCard(props: MagicCardProps) {
  const {
    ...
    className,
    gradientSize = 200,
    gradientColor = "rgba(167, 218, 219, 0.15)", // BRAND_TEAL
    gradientOpacity = 0.8,
    gradientFrom = "rgba(167, 218, 219, 0.4)",
    gradientTo = "transparent",
    mode = "gradient",
  } = props

  const glowFrom = isOrbMode(props) ? (props.glowFrom ?? "#a7dadb") : "#a7dadb"
  const glowTo = isOrbMode(props) ? (props.glowTo ?? "#4F46E5") : "#4F46E5"
  const glowAngle = isOrbMode(props) ? (props.glowAngle ?? 90) : 90
  const glowSize = isOrbMode(props) ? (props.glowSize ?? 420) : 420
  const glowBlur = isOrbMode(props) ? (props.glowBlur ?? 60) : 60
  const glowOpacity = isOrbMode(props) ? (props.glowOpacity ?? 0.9) : 0.9
  
  // We assume dark theme for this project
  ...
  return (
    <motion.div
      className={cn(
        "group relative isolate overflow-hidden rounded-[24px] border border-transparent",
        className
      )}
      ...
      style={{
        background: useMotionTemplate`
          linear-gradient(#020C1B 0 0) padding-box,
    ...
    >
      <div className="bg-[#020C1B] absolute inset-px z-20 rounded-[inherit] opacity-80" />
