...
function getProgramInfo(gl: WebGLRenderingContext, program: WebGLProgram): ProgramInfo | null {
  ...
  return {
    attributeLocation,
    program,
    uniforms: { angle, cellSize, containerSize, devicePixelRatio, lineColor, time, viewportSize },
  }
}
...
export function RetroGrid({
  ...
  useEffect(() => {
    ...
    const handleContextLost = (event: Event) => {
      event.preventDefault(); isContextLost = true; stopAnimation(); releasePipeline(false); setIsWebGlReady(false)
    }
    ...
    return () => {
      stopAnimation(); resizeObserver.disconnect(); intersectionObserver.disconnect(); themeObserver.disconnect()
  ...
  const fallbackProjectionStyles = { perspective: `${PERSPECTIVE_PX}px` } as CSSProperties
  const fallbackRotationStyles = { transform: `rotateX(${normalizedAngle}deg)` } as CSSProperties
  ...
  return (
    <div ref={containerRef} className={cn("pointer-events-none absolute size-full overflow-hidden", className)} style={gridStyles} {...props}>
      ...
      <canvas ref={canvasRef} className={cn("absolute inset-0 size-full", isWebGlReady ? "opacity-100" : "opacity-0")} />
      <div className="absolute inset-0 bg-gradient-to-t from-[#020C1B] to-transparent to-90%" />
    </div>
  )
}
